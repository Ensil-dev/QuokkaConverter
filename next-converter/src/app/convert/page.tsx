...
export default function Home() {
  ...
  const convertFile = useCallback(
    async (inputFile: File, targetFormat: string, options: Record<string, unknown> = {}) => {
      ...
      try {
        ...
        throw new Error(
          `파일 변환에 실패했습니다: ${error instanceof Error ? error.message : '알 수 없는 오류'}`
        );
      }
    },
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      ...
      router.push(
        `/result?url=${encodeURIComponent(resultInfo.url)}&filename=${encodeURIComponent(resultInfo.filename)}&size=${resultInfo.size}`
      );
    } catch (error) {
      console.error('변환 오류:', error);

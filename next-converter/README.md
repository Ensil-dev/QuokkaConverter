...
- **관리자 탭 제어**: `ALLOWED_EMAILS`로 지정한 이메일만 관리자 페이지 접근
...
- 모든 사용자는 로그인 후 기능을 사용할 수 있습니다.
- `ALLOWED_EMAILS` 환경변수로 등록한 이메일만 Admin 페이지에 접근할 수 있습니다.

### 파일 크기 제한

- 기본: 100MB (관리자 페이지에서 조정 가능)
...
1. **파일 크기 제한**: 관리자 페이지에서 적절히 설정
2. **변환 시간 제한**: 5분 이하로 설정
3. **관리자 이메일 제한**: 관리자 기능을 필요한 계정에만 허용
